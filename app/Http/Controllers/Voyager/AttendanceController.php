...
use App\Http\Resources\Attendance as ResourcesAttendance;
...
class AttendanceController extends VoyagerBaseController
{
    ...
    public function oneEmployeesReport(Request $request)
    {
        ...
        if ($request->emp_id && $request->emp_id != null) {
            $empName = \App\Models\User::find($request->emp_id)->name;
            $salary = \App\Models\User::find($request->emp_id)->salary;
        }

        $fromDate = null;
        $toDate = null;

        // dd(DATE($request->from_date), DATE($request->to_date));
        ...
        $periods =  Period::where('company_id', Auth::user()->company_id)->get();
        ...
        foreach ($attendance as $key => $value) {
            ...
            $obj->delay_time  = $value->delay_time;
            $obj->zzzz = $value->period->from_time;
            $obj->status = $value->status;

            $obj->number_days_in_month = cal_days_in_month(CAL_GREGORIAN, date('m', strtotime($value->attendance_time)), date('Y', strtotime($value->attendance_time)));;

            $obj->periodData = Period::find($value->period_id);

            $periodFromTime = new DateTime(date('H:i', strtotime(Period::find($value->period_id)->from_time)));
            $periodToTime = new DateTime(date('H:i', strtotime(Period::find($value->period_id)->to_time)));
            $intervalTime = $periodFromTime->diff($periodToTime);
            $obj->hours = date('H:i:s',  strtotime($intervalTime->h . ':' . $intervalTime->i));

            $obj->period_name = $value->period->name;
            if ($value->attendance_type == 'attending') {
                $attedingTime = new DateTime(date('H:i', strtotime($value->attendance_time)));
                $periodFromTime = new DateTime($value->period->from_time);
                $interval = $attedingTime->diff($periodFromTime);
                $obj->delay_duration = $interval->h . ':' . $interval->i;
            }

            $attendanceResult[$obj->attendance_date . '-' . $value->period_id][] = $obj;
        }
        ...
        return view(
            'voyager::attendance.reports.attendance-one-employe-report',
            compact('attendanceResult', 'empName', 'salary', 'toDate', 'fromDate')
        );
    }
    public function allEmployeesReport(Request $request)
    {
        ...
        // $date = date('Y-m-d', strtotime("2022-08-01"));
        // echo  $date;
        // dd($date);
        // dd(new DateTime(date('Y-m-d', strtotime("2022-08-01"))));
        // dd(new DateTime($date)->format('Y-m-d'));
    ...
    public function report(Request $request)
    {
        ...
        $periods =  Period::where('company_id', Auth::user()->company_id)->get();
        $attendance = Attendance::where('emp_id', $request->emp_id)
            ->whereBetween('attendance_time', [DATE($request->from_date),  DATE($request->to_date)])
            ->where('company_id', Auth::user()->company_id)->get();
        ...
        return view(
            'voyager::attendance.reports.attendance-report',
            compact('periods', 'attendanceResult', 'empName')
        );
    }
    ...
    public function reportEmps(Request $request)
    {
        ...
        foreach ($dates as   $val_date) {
            foreach ($employees as $key => $value) {
                ...
                foreach (Period::where('company_id', Auth::user()->company_id)->get() as   $per_val) {
                    $attendance = Attendance::where('emp_id', $emp_id)
                        ->where('period_id', $per_val->id)
                        ->whereDate('attendance_time', '=', $val_date->format('Y-m-d'))
                        ->where('company_id', Auth::user()->company_id)->get();
                    ...
                    foreach ($attendance as $key => $value) {
                        ...
                        if ($value->attendance_type == 'leaving') {
                            $obj->early_leaving = $value->early_leaving;
                        }
        ...
        return view('voyager::attendance.reports.emps-attendance-report', compact('result', 'to_date', 'from_date'));
    }

    public function reportEmpsHours(Request $request)
    {
        ...
        foreach ($employees as $key => $value) {
            ...
            $result[] = [
                'emp_id' => $emp_id,
                'emp_name' => $emp_name,
                'hours_work' =>  $this->getEmpHours($request, $emp_id)['work_hours'],
                'delay_hours' =>   $this->getEmpHours($request, $emp_id)['delay_hours'],
                'early_leaving' =>  $this->getEmpHours($request, $emp_id)['early_leaving'],
                'requested_work_hours' => $this->getRequestedWorkHours($request, $emp_id)
            ];
        }

        return view('voyager::attendance.reports.emps-attendance-report-hours', compact('result', 'monthName'));
    }


    public function getRequestedWorkHours($request, $emp_id)
    {
        ...
        if (User::find($emp_id)->number_of_hours != null) {
            $requested_work_hours = User::find($emp_id)->number_of_hours;
        } else {
    ...
    public function  getEmpHours($request, $emp_id)
    {
        ...
        $startMonth = date('Y-m-d', strtotime('first day of ' . $month . ' ' . $year));
        $lastMonth = date('Y-m-d', strtotime('last day of ' . $month . ' ' . $year));

        // dd($startMonth, $lastMonth);
        $data = Attendance::where('emp_id', $emp_id)
            ->whereBetween('attendance_time', [DATE($startMonth),  DATE($lastMonth)])
            ->get();
