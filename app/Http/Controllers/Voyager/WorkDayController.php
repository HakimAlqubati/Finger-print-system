...
class WorkDayController extends VoyagerBaseController
{
    ...
    public function createPeriods(Request $request, $id)
    {
        $dayName = WorkDay::find($id)->name;

        return view(
            'voyager::work-days.work-day-period',
            compact('id','dayName')
        );
    }

    public function addPeriods(Request $request, $id)
    {
        // dd($request->all()); 
        foreach ($request->periods as   $period_id) {
            $add =   WorkDayPeriod::create([
                'period_id' => $period_id,
                'work_day_id' => $id
            ]);
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->code == 'Sat') {
            $name = 'السبت';
        } else if ($request->code == 'Sun') {
            $name = 'الأحد';
        } else if ($request->code == 'Mon') {
            $name = 'الإثنين';
        } else if ($request->code == 'Tue') {
            $name = 'الثلاثاء';
        } else if ($request->code == 'Wed') {
            $name = 'الأربعاء';
        } else if ($request->code == 'Thu') {
            $name = 'الخميس';
        } else if ($request->code == 'Fri') {
            $name = 'الجمعة';
        }

        $branch = WorkDay::create([
            'name' => $name,
            'code' => $request->code,
