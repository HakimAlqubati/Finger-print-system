...
class UserController extends VoyagerBaseController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->id_image) {
            ...
            Storage::disk('local')->put('public/users/identities' . '/' . $fileName, $img, 'public');
            $idImageName = 'users/identities/'  . $fileName;
        }
        ...
        if ($request->no_fingerprint_tracking == "on") {
            $no_fingerprint_tracking = 1;
        } else {
            $no_fingerprint_tracking = 0;
        }

        if ($request->no_attendance_tracking == "on") {
            $no_attendance_tracking = 1;
        } else {
        ...
        $user = User::find($id)->update(
            [
                'name' => $request->name,
                'email' => $request->email,
                'password' => $request->password != null ?  bcrypt($request->password) : User::find($id)->password,
