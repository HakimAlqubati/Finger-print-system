...
class AttendanceSettingsController extends VoyagerBaseController
{
    ...
    public function printBarcode(Request $request, $id)
    {
        ...
        return view(
            'voyager::attendance-settings.print-barcode',
            compact('qr_code', 'branch')
        );
    }
    ...
    public function store(Request $request)
    {
        ...
        $attendaceSettings = AttendanceSetting::create([
            'title'  => $request->title,
            // 'qr_code' => $request->qr_code,
            'qr_code' =>  $this->generateRandomString(50),
            'first_location' => $request->first_location,
            'second_location' => $request->second_location,
            'third_location' => $request->third_location,
            'ford_location' => $request->ford_location,
            'branch_id' => $request->branch_id,
            'company_id' => $request->company_id
        ]);

        if ($attendaceSettings) {
            $redirect = redirect()->route("voyager.{$dataType->slug}.index");
            return $redirect->with([
                'message'    =>
                __('voyager::generic.successfully_added_new')
                    .
