...
@section('content')
                                        ...
                                        @if ($data->company_id == Auth::user()->company_id)
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    @php
                                                        $periodsCount = \App\Models\Period::get()->count();
                                                    @endphp
                                                    @if ($periodsCount < 1)
                                                        <a href="{{ url('/admin/periods/create') }}"
                                                            class="btn btn-success">
                                                            إضافة فترات الدوام
                                                        </a>
                                                    @else
                                                        <a href="{{ url('/admin/add-periods-to-days/' . $data->getKey()) }}"
                                                            class="btn btn-success">
                                                            إضافة الفترات
                                                            <i class="voyager-plus"></i> <span></span>
                                                        </a>
                                                    @endif
                                                    {{-- <button type="button" class="btn btn-success btn-add-periods"
                                                        id="{{ $data->getKey() }}">
                                                        إضافة الفترات
                                                        <i class="voyager-plus"></i> <span></span>
                                                    </button> --}}
                                                ...
                                                <td>
                                                    ...
                                                    @php
                                                        $periods = \App\Models\WorkDayPeriod::where('work_day_id', $data->getKey())->get();
                                                        foreach ($periods as $key => $value) {
                                                            // echo $value->period_id . '<br>';
                                                            echo \App\Models\Period::find($value->period_id)->name . ', ';
                                                        }
                                                    @endphp
    ...
    <div id="dialog-modal"
        style=" text-align: center;  background: #fafafa; border: 4px solid #755245;
                                                    border-radius: 20px;
                                                    padding: 20px;
                                                ">
        <form action="">
            ...
            <button type="submit" class="btn btn-primary" id="addPeriodsAjax">حفظ</button>
        </form>
    </div>
...
@section('javascript')
    ...
    <script>
        $(document).ready(function() {
            ...
            $(".btn-add-periods").click(function() {
                var id = $(this).attr('id')
                $('#dialog-modal')
                    .data('work_day_id', id)
                    .dialog('open');
            ...
            $("#addPeriodsAjax").click(function(e) {
                var i = 0;
                var arr = [];
                $('.periods:checked').each(function() {
                    arr[i++] = $(this).val();
                });
                console.log('ghareeb===>', arr)
                e.preventDefault();
                $.ajax({
                    url: '/emp-finger-print/add-periods-to-days/' + $("#dialog-modal").data(
                        'work_day_id'),
                    type: "POST",
            ...
            // jQuery('#addPeriodsAjax').click(function(e) {
            //     var i = 0;
            //     var arr = [];
            //     $('.periods:checked').each(function() {
            //         arr[i++] = $(this).val();
            //     });
            //     console.log('hakeem', arr)
            //     e.preventDefault();
            //     jQuery.ajax({ 
            //         url: '/emp-finger-print/add-periods-to-days/' + $("#dialog-modal").data(
            //             'work_day_id'),
